...
import { FormResidentsAppointmentComponent } from '../../components/form-residents-appointment/form-residents-appointment.component';
...
export class ResidentsAppointmentsEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (appointmentId && residentId) {
      this.subs.push(
        this.residentAppointmentsService
          .fetchItem(appointmentId, residentId)
          .subscribe({
            next: (appointment) => {
  ...
  onFormSubmit(appointmentDTO: AppointmentDTO) {
    if (!this.appointment?.id || !this.resident?.id || this.isSubmitting)
      return;
    ...
    this.subs.push(
      this.residentAppointmentsService
        .update(this.appointment.id, appointmentDTO, this.resident.id)
        .subscribe({
          next: () =>
            this.router.navigate([
              `/residents/${this.resident?.id}/appointments`,
            ]),
          error: (err) => {
