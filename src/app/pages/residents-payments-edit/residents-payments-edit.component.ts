...
constructor(
  ...
  ngOnInit() {
    ...
    if (paymentId && residentId) {
      this.subs.push(
        this.residentPaymentsService.fetchItem(paymentId, residentId).subscribe({
          next: (payment) => {
  ...
  onFormSubmit(paymentDTO: PaymentDTO) {
    ...
    this.subs.push(
      this.residentPaymentsService.update(
        this.payment.id, paymentDTO, this.resident.id
      ).subscribe({
        next: () => this.router.navigate([`/residents/${this.resident?.id}/payments`]),
        error: (err) => {
